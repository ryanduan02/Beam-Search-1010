...
def clear_lines(board: Board) -> tuple[Board, int, int, int]:
    ...
    full_rows = [r for r in range(height) if all(board[r][c] != 0 for c in range(width))]
    full_cols = [c for c in range(width) if all(board[r][c] != 0 for r in range(height))]
...
def apply_move(state: GameState, hand_index: int, row: int, col: int) -> Tuple[GameState, int, int, int]:
    ...
    new_score = state.score + score_delta(piece, cleared_cells)

    new_hand = state.hand[:hand_index] + state.hand[hand_index + 1 :]
    return GameState(board=new_board, score=new_score, hand=new_hand), cleared_cells, rows_cleared, cols_cleared
